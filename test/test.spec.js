var justifiedLayout = require("../dist/better-justified-layout.js");
...
function isThisWidowRowCentered(layout) {
  ...
  for (
    n = totalBoxCount - 1;
    n >= totalBoxCount - geometryCenteredWidows.widowCount;
    n--
  ) {
  ...
  leftOfFirstWidow =
    geometryCenteredWidows.boxes[
      geometryCenteredWidows.boxes.length - geometryCenteredWidows.widowCount
    ].left;
...
describe("better-justified-layout", function () {
  test("should create additional rows if it won't fit within constraints", function () {
  ...
  test("should not add the row if we are limiting it with maxNumRows", function () {
  ...
  test("should handle a panorama as only row item", function () {
    var geometry = justifiedLayout([5]);

    expect(geometry.boxes.length).toEqual(1);
  });

  test("should allow new item added to the row to get closer to the targetRowHeight", function () {
...
describe("input", function () {
  test("should handle width and height objects as input", function () {
    expect(
      justifiedLayout([
        {
          width: 400,
  ...
  test("should handle an array of aspect ratios as input", function () {
    expect(justifiedLayout([1, 1, 1, 1])).toEqual(fourSquares);
  });

  test("should error if box height not passed in", function () {
    try {
      justifiedLayout([{ width: 10 }]);
    } catch (e) {
      expect(e.message).toEqual("Item 0 has an invalid aspect ratio");
    }
  });

  test("should error if box height not passed in", function () {
    try {
      justifiedLayout([{ height: 10 }]);
    } catch (e) {
      expect(e.message).toEqual("Item 0 has an invalid aspect ratio");
    }
  });

  test("should error if aspect ratio is not a number", function () {
    try {
      justifiedLayout(["NaN"]);
    } catch (e) {
      expect(e.message).toEqual("Item 0 has an invalid aspect ratio");
...
describe("config", function () {
  test("should return a layout without passing in a config", function () {
    expect(justifiedLayout([1, 1, 1, 1])).toEqual(fourSquares);
  });

  test("should allow overriding of containerWidth", function () {
    expect(
  ...
  test("should allow overriding of containerPadding", function () {
  ...
  test("should allow overriding of containerPadding with multiple dimensions", function () {
    var geometry = justifiedLayout([1], {
      containerPadding: {
        top: 50,
        left: 5,
  ...
  test("should allow overriding of boxSpacing", function () {
  ...
  test("should allow overriding of boxSpacing with multiple dimensions", function () {
    var geometry = justifiedLayout([1, 1, 1, 1], {
      boxSpacing: {
        horizontal: 40,
        vertical: 5,
  ...
  test("should allow overriding of targetRowHeight", function () {
    var geometry = justifiedLayout([1, 1, 1, 1], {
      targetRowHeight: 255,
  ...
  test("should allow overriding of targetRowHeightTolerance", function () {
  ...
  test("should allow overriding of maxNumRows", function () {
  ...
  test("should allow overriding of forceAspectRatio", function () {
  ...
  test("should allow overriding of showWidows", function () {
    var geometry = justifiedLayout([1, 1, 1, 1], {
      showWidows: false,
    });
  ...
  test("should allow overriding of fullWidthBreakoutRowCadence", function () {
    var geometry = justifiedLayout([1, 1, 1, 1, 2, 2, 2, 2], {
      fullWidthBreakoutRowCadence: 3,
    });

    expect(geometry.boxes[5].width).toEqual(1040);
    expect(geometry.boxes[6].top).toEqual(
      geometry.boxes[5].top + geometry.boxes[5].height + 10,
    );
  });
});

describe("widows", function () {
  test("should set them at the same height as previous rows which looks nicer", function () {
    var geometry = justifiedLayout([1, 1, 1, 1]);

    expect(geometry.boxes[0].height).toEqual(geometry.boxes[3].height);
  });

  test("should set them at the same height as previous non-breakout row", function () {
  ...
  test("should return 0 value widowCount property if there are not any", function () {
    var geometryNoWidows = justifiedLayout([1, 1, 1]);

    expect(geometryNoWidows.widowCount).toEqual(0);
  });

  test("should return the number of widowCount in widows property if there are widows", function () {
  ...
  test("should return widows with a left layout through the default", function () {
    var geometry1LeftWidow = justifiedLayout([1, 1, 1, 1]);

    expect(geometry1LeftWidow.boxes[0].left).toEqual(
      geometry1LeftWidow.boxes[3].left,
    );
  });

  test("should return widows with a specified left layout", function () {
    var geometry1LeftWidow = justifiedLayout([1, 1, 1, 1], {
  ...
  test("should return single widow with justified layout", function () {
    ...
    var widthOfFinalJustifiedItem =
      geometryJustfiedWidows.boxes[geometryJustfiedWidows.boxes.length - 1]
        .width;
  ...
  test("should return widows left aligned if a nonsense value is provided", function () {
    var boxSpacing = 10;
...
describe("containerPadding", function () {
  test("should add padding to the bottom of the container too", function () {
    var geometry = justifiedLayout([1], {
  ...
  test("should handle 0 padding", function () {
    var geometry = justifiedLayout([1, 1, 1], {
      containerPadding: 0,
      targetRowHeightTolerance: 0,
    });
